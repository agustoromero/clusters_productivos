...
def build_features(estab_rows, level_geo, level_activity):
    ...
    for k, vals in by_cell.items():
        ...
        row = {
            ...
            "salario_promedio_anual_deflactado": round(vals["salario_promedio_anual_deflactado"], 2),
...
def build_gender_features(gender_rows, level_geo, level_activity):
    gsum = defaultdict(lambda: {"mujeres": 0.0, "varones": 0.0, "otro": 0.0, "salario_promedio_anual_deflactado": 0.0, "meses_completos": 0})
    ...
    for r in gender_rows:
        ...
        gsum[k]["salario_promedio_anual_deflactado"] += to_float(r.get("salario_promedio_anual_deflactado", 0.0))
        gsum[k]["meses_completos"] += to_int(r.get("meses_completos", 0))
